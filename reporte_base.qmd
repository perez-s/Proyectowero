...
# firstReportTable <- data.frame(Ciudad = character(), `Cantidad RS (Kg)` = numeric(), Recolecciones = numeric(), `Kg/recolección` = numeric(), check.names = F ) 
...
#   firstReportTable <- rbind(    
#     firstReportTable, data.frame(
#       Ciudad = ciudadesList[i],
#       `Cantidad RS (Kg)` = totalKg,
#       Recolecciones = numRecolecciones,
#       `Kg/recolección` = kgPerRecoleccion,
#       check.names = F,
...
firstReportTable |>
  ...
  tab_options(
    ...
    table_body.vlines.color = "black"  # You can also use `pt(10)` or `pct(80)`
  )
...
#set table(stroke: (x, y) => (
  left: if x == 0 or y > 0 { 0pt } else { 0pt },
  right: 0pt,
  top: if y == 1 { 0.5pt } else if y == 0 { 0.5pt } else if y == 2 { 0.5pt } else { 0pt },
  bottom: 0.5pt,
))
#set text(8pt)
...
secondReportTable |>
  ...
  tab_spanner(
    label = last_4_digits[1],
    columns = ends_with(last_4_digits[1])
  )|>
  tab_spanner(
    label = last_4_digits[2],
    columns = ends_with(last_4_digits[2])
  )|>
  cols_label_with(
    fn = ~ ifelse(grepl("\\d{4}$", .), substr(., 1, nchar(.) - 5), .)  # Remove last 4 characters from column names
  )|>
  fmt_number(
    decimals = 0,
    use_seps = FALSE
  ...
  tab_style(
    ...
    locations = list(
      cells_column_labels(columns = ends_with(paste("dic", sep = " ", last_4_digits[1]))),
      cells_body(columns = ends_with(paste("dic", sep = " ", last_4_digits[1]))),
      cells_column_spanners(spanners = ends_with(last_4_digits[1])),
      cells_grand_summary(columns = ends_with(paste("dic", sep = " ", last_4_digits[1]))),
  ...
  tab_options(
    table.font.size = px(10.5),
    table.font.names = "Montserrat"  # You can also use `pt(10)` or `pct(80)`
  )
``` 
...
thirdReportTable |>
  ...
  cols_label_with(
    fn = ~ ifelse(grepl("\\d{4}$", .), substr(., 1, nchar(.) - 4), .)  # Remove last 4 characters from column names
  )|>
  fmt_number(
    decimals = 0,
    use_seps = FALSE
...
for (i in 1:length(ciudadesList)) {
  ...
  kgPerRecoleccion <- totalKg/numRecolecciones
  if (i == length(ciudadesList) & length(ciudadesList) > 1) {
    largestring2 <- paste(largestring2,sprintf(" y %s kg de residuos en la ciudad de %s",format(round(kgPerRecoleccion,2), nsmall=2),ciudadesList[i]), sep = "")
  }

  else if (length(ciudadesList) == 1 | length(ciudadesList) == 2 | i == length(ciudadesList)-1) {
    largestring2 <- paste(largestring2,sprintf(" %s kg de residuos en la ciudad de %s",format(round(kgPerRecoleccion,2), nsmall=2),ciudadesList[i]), sep = "")
  }
...
(total: `{r} format(round(sum(newdata33$Peso)),2)` kg)
