...
if ss["authentication_status"]:
    ...
    startdate = enddate - relativedelta(months=13)
    ...
    recomendaciones = {
        "Recomendación  1": "Durante las semanas de diciembre de 2023 y los primeros meses de 2024, se ha observado una continuidad en la presencia de grandes cantidades de material no aprovechable o material de rechazo en los puntos de recolección. Entre estos residuos, se incluyen plásticos contaminados con grasas y restos orgánicos de las proteínas, restos de comida preparada y no preparada, servilletas de cocina y de mesa, así como una mezcla de materiales reciclables con no reciclables que afecta la calidad del material recuperable. Esta situación limita la eficiencia del proceso de reciclaje y representa un desafío significativo en la correcta segregación de residuos.\n\nAdemás, los nuevos puntos de recolección implementados como parte del piloto, junto con los retos inherentes a la separación en la fuente de los residuos sólidos, evidencian la necesidad urgente de reforzar las estrategias de educación y concientización. Desde el área de sostenibilidad de Qbano, se han llevado a cabo diversas acciones de comunicación y capacitación a través de WERO, lo que ha favorecido una mayor apropiación por parte de los usuarios de las pautas de clasificación. Estos esfuerzos han resultado en una mejora en la separación de materiales reciclables y no reciclables. No obstante, el desafío persiste y continúa siendo una prioridad garantizar la efectividad del sistema de recolección.\n\nPara fortalecer la separación en la fuente en cada punto de venta de las distintas ciudades, se recomienda enfocar la estrategia en dos acciones clave- la creación de un concurso de reciclaje y la implementación de campañas de sensibilización. El concurso deberá premiar a los puntos de venta que demuestren un alto nivel de cumplimiento en la correcta separación de residuos, fomentando la motivación tanto entre los colaboradores como entre los consumidores para que se involucren activamente en la correcta separación en la fuente.\n\nAdemás, se debe ampliar el alcance de las campañas de sensibilización, utilizando medios digitales para llegar tanto a los colaboradores como a los consumidores, a través de videos tutoriales y mensajes interactivos que refuercen la importancia de la separación de residuos y su impacto positivo en el ambiente. Esta combinación de incentivos y educación podrá contribuir a una mayor conciencia y participación en el proceso de segregación en la fuente."
}
    recomendacioneslist = st.selectbox("Selecciona una recomendación:", list(recomendaciones.keys()))
    recomendacion2 = repr(recomendaciones[recomendacioneslist]).replace("'", '"')
    ...
    if st.button("Generar Reporte", key="generate_report", use_container_width=True):
        with st.spinner(f"Generando reporte de {Clientes}...", show_time=True):
            ...
            if oganicosselect == "Si":
                os.system(f"quarto render reporte_organicos.qmd -P obs:{obsevacion2} -P rec:{recomendacion2} -P nodoc:{nodoc} -P client:\"{Clientes}\" -P startdate:{startdate} -P enddate:{enddate} --output \"{Clientes}\".pdf")    
            else:
                os.system(f"quarto render reporte_base.qmd -P obs:{obsevacion2} -P rec:{recomendacion2} -P nodoc:{nodoc} -P client:\"{Clientes}\" -P startdate:{startdate} -P enddate:{enddate} --output \"{Clientes}\".pdf")
            try:    
